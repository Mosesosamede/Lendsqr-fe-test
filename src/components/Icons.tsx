import React from 'react';

export const Logo: React.FC<{ className?: string; style?: React.CSSProperties }> = ({ className, style }) => (
  <svg width="174" height="36" viewBox="0 0 174 36" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} style={style}>
    <path fillRule="evenodd" clipRule="evenodd" d="M0 2.66541C0 2.11304 0.443238 1.66525 0.99 1.66525H3.96C4.50676 1.66525 4.95 2.11304 4.95 2.66541V20.6682C4.95 21.2206 5.39324 21.6684 5.94 21.6684H18.81C19.3568 21.6684 19.8 21.2206 19.8 20.6682V17.6678C19.8 17.1154 19.3568 16.6676 18.81 16.6676H12.87C11.2297 16.6676 9.9 15.3243 9.9 13.6671V4.66573C9.9 3.00861 11.2297 1.66525 12.87 1.66525H21.78C23.4203 1.66525 24.75 3.00861 24.75 4.66573V23.6687C24.75 25.3258 23.4203 26.6692 21.78 26.6692H2.97C1.32971 26.6692 0 25.3258 0 23.6687V2.66541ZM19.8 7.6662C19.8 7.11383 19.3568 6.66604 18.81 6.66604H14.85V10.6667C14.85 11.219 15.2932 11.6668 15.84 11.6668H19.8V7.6662Z" fill="url(#paint0_linear)"/>
    <path d="M35.325 0V27.8055H40.0894V0H35.325Z" fill="#213F7D"/>
    <path d="M63.5388 19.7304C64.2869 11.6552 60.2313 7.59779 53.7344 7.59779C47.4344 7.59779 43.4181 11.8939 43.4181 17.8608C43.4181 24.1459 47.395 28.3227 54.0888 28.3227C57.0419 28.3227 60.3888 27.2884 62.4363 25.1403L59.365 22.0773C58.2625 23.2309 55.9394 23.9072 54.1675 23.9072C50.7813 23.9072 48.6944 22.1569 48.3794 19.7304H63.5388ZM48.4581 15.7127C49.1669 13.1669 51.2931 11.8939 53.8919 11.8939C56.6481 11.8939 58.6169 13.1669 58.9319 15.7127H48.4581Z" fill="#213F7D"/>
    <path d="M86.3372 27.8055V17.5028C86.3372 11.337 82.6753 7.87624 78.1078 7.87624C75.6666 7.87624 73.6978 8.87072 71.7291 10.8199L71.4141 8.1547H67.1222V27.8055H71.8866V17.7812C71.8866 14.7978 73.8947 12.2917 76.8478 12.2917C79.9191 12.2917 81.5334 14.5591 81.5334 17.5425V27.8055H86.3372Z" fill="#213F7D"/>
    <path d="M99.876 12.2519C102.908 12.2519 105.507 14.5591 105.507 17.9801C105.507 21.5204 102.908 23.7481 99.876 23.7481C96.8048 23.7481 94.3635 21.4011 94.3635 17.9801C94.3635 14.4398 96.8048 12.2519 99.876 12.2519ZM105.822 0V10.8597C104.68 8.83094 101.49 7.71713 99.4429 7.71713C93.7729 7.71713 89.5598 11.2177 89.5598 17.9801C89.5598 24.4243 93.8517 28.2431 99.561 28.2431C101.924 28.2431 104.325 27.4475 105.822 25.1006L106.137 27.8055H110.625V0H105.822Z" fill="#213F7D"/>
    <path d="M130.162 10.2232C127.917 8.23425 125.634 7.63757 122.602 7.63757C119.058 7.63757 114.412 9.22873 114.412 13.7635C114.412 18.2188 118.782 19.5713 122.444 19.8497C125.201 20.0088 126.303 20.5657 126.303 21.9182C126.303 23.3503 124.61 24.305 122.799 24.2652C120.633 24.2254 117.483 23.0718 116.105 21.5204L113.742 24.9812C116.577 27.9646 119.649 28.4818 122.72 28.4818C128.272 28.4818 131.067 25.4983 131.067 22.0376C131.067 16.8265 126.421 15.9912 122.759 15.7525C120.279 15.5934 119.137 14.8773 119.137 13.6044C119.137 12.3713 120.397 11.6552 122.681 11.6552C124.531 11.6552 126.106 12.0928 127.484 13.4055L130.162 10.2232Z" fill="#213F7D"/>
    <path d="M143.518 23.5889C140.25 23.5889 138.006 21.0829 138.006 18.0199C138.006 14.9569 140.053 12.4508 143.518 12.4508C146.983 12.4508 149.031 14.9569 149.031 18.0199C149.031 21.0829 146.786 23.5889 143.518 23.5889ZM154.268 36V8.19448H149.779L149.464 10.8994C147.968 8.7116 145.369 7.75691 143.164 7.75691C137.179 7.75691 133.202 12.2519 133.202 18.0199C133.202 23.7481 136.785 28.2829 143.006 28.2829C145.054 28.2829 148.086 27.6464 149.464 25.4983V36H154.268Z" fill="#213F7D"/>
    <path d="M158.487 8.19448V27.8055H163.291V17.463C163.291 13.8431 165.614 12.4906 168.134 12.4906C169.709 12.4906 170.615 12.9282 171.599 13.7635L173.765 9.54696C172.702 8.47293 170.772 7.67735 168.764 7.67735C166.795 7.67735 164.787 8.03536 163.291 10.4619L162.937 8.19448H158.487Z" fill="#213F7D"/>
    <defs>
      <linearGradient id="paint0_linear" x1="-1.87183e-06" y1="38.9211" x2="26.2531" y2="-4.22382" gradientUnits="userSpaceOnUse">
        <stop stopColor="#213F7D"/>
        <stop offset="1" stopColor="#39CDCC"/>
      </linearGradient>
    </defs>
  </svg>
);

export const UsersIcon: React.FC = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M16 14V16C16 17.6569 14.6569 19 13 19H4C2.34315 19 1 17.6569 1 16V14" stroke="#DF18FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M8.5 13C11.2614 13 13.5 10.7614 13.5 8C13.5 5.23858 11.2614 3 8.5 3C5.73858 3 3.5 5.23858 3.5 8C3.5 10.7614 5.73858 13 8.5 13Z" stroke="#DF18FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M20 19V17C20 15.3431 18.6569 14 17 14" stroke="#DF18FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M16.5 3.09998C18.5 3.79998 20 5.69998 20 7.99998C20 10.3 18.5 12.2 16.5 12.9" stroke="#DF18FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);

export const ActiveUsersIcon: React.FC = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M16 14V16C16 17.6569 14.6569 19 13 19H4C2.34315 19 1 17.6569 1 16V14" stroke="#5718FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M8.5 13C11.2614 13 13.5 10.7614 13.5 8C13.5 5.23858 11.2614 3 8.5 3C5.73858 3 3.5 5.23858 3.5 8C3.5 10.7614 5.73858 13 8.5 13Z" stroke="#5718FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M20 19V17C20 15.3431 18.6569 14 17 14" stroke="#5718FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M16.5 3.09998C18.5 3.79998 20 5.69998 20 7.99998C20 10.3 18.5 12.2 16.5 12.9" stroke="#5718FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);

export const LoansIcon: React.FC = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M22 20V4" stroke="#F55F44" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M17 20V10" stroke="#F55F44" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M12 20V6" stroke="#F55F44" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M7 20V14" stroke="#F55F44" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M2 20H22" stroke="#F55F44" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);

export const SavingsIcon: React.FC = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M19 6C19 4.34315 17.6569 3 16 3H8C6.34315 3 5 4.34315 5 6V19C5 20.6569 6.34315 22 8 22H16C17.6569 22 19 20.6569 19 19V6Z" stroke="#FF3366" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M12 7V11" stroke="#FF3366" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M10 9H14" stroke="#FF3366" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);

export const FilterIcon: React.FC = () => (
  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M14.6667 2H1.33333L6.66667 8.30667V12.6667L9.33333 14V8.30667L14.6667 2Z" stroke="#545F7D" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);

export const OptionsIcon: React.FC = () => (
  <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M10 11.6667C10.9205 11.6667 11.6667 10.9205 11.6667 10C11.6667 9.0795 10.9205 8.33333 10 8.33333C9.0795 8.33333 8.33333 9.0795 8.33333 10C8.33333 10.9205 9.0795 11.6667 10 11.6667Z" fill="#545F7D"/>
    <path d="M10 5.83333C10.9205 5.83333 11.6667 5.08714 11.6667 4.16667C11.6667 3.24619 10.9205 2.5 10 2.5C9.0795 2.5 8.33333 3.24619 8.33333 4.16667C8.33333 5.08714 9.0795 5.83333 10 5.83333Z" fill="#545F7D"/>
    <path d="M10 17.5C10.9205 17.5 11.6667 16.7538 11.6667 15.8333C11.6667 14.9128 10.9205 14.1667 10 14.1667C9.0795 14.1667 8.33333 14.9128 8.33333 15.8333C8.33333 16.7538 9.0795 17.5 10 17.5Z" fill="#545F7D"/>
  </svg>
);